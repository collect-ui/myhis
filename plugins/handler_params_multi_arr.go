...
func (si *MultiArr) HandlerData(template *config.Template, handlerParam *config.HandlerParam, ts *templateService.TemplateService) *common.Result {
	...
	// loop_max 模式：不传 right，直接把 foreach 数组的每个元素复制 N 份
	if handlerParam.LoopMax > 0 {
		n := int(handlerParam.LoopMax)
		target := make([]map[string]interface{}, len(arr)*n)
		for i, fItem := range arr {
			for j := 0; j < n; j++ {
				targetItem := utils.Copy(fItem).(map[string]interface{})
				targetItem["_seq"] = j + 1
				target[len(arr)*i+j] = targetItem
			}
		}
		return common.Ok(target, "处理参数成功")
	}
