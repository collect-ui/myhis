# template function=backend_encrypt
...
- 在 `params.*.template` 中生成 `backend_encrypt` 相关值。
...
### 参数
- 参数形式以实现类 `BackendEncrypt` 的 `filter` 方法为准。
...
params:
  demo:
    template: '{{backend_encrypt .value}}'
```

## 注意事项

### 定位
- 悬停命中文档路径：`tooltip-docs/python/key/template/value/backend_encrypt.md`
