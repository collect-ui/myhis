...
- 在 `params.*.template` 中生成 `is_number` 相关值。
...
### 参数
- 参数形式以实现类 `IsNumberFilter` 的 `filter` 方法为准。
...
params:
  demo:
    template: '{{is_number .value}}'
```

## 注意事项

### 定位
- 悬停命中文档路径：`tooltip-docs/python/key/template/value/is_number.md`
