...
- 在 `params.*.template` 中生成 `clock_time` 相关值。
...
### 参数
- 参数形式以实现类 `ClockTime` 的 `filter` 方法为准。
...
params:
  demo:
    template: '{{clock_time .value}}'
```

## 注意事项

### 定位
- 悬停命中文档路径：`tooltip-docs/python/key/template/value/clock_time.md`
