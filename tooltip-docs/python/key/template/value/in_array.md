...
- 在 `params.*.template` 中生成 `in_array` 相关值。
...
### 参数
- 参数形式以实现类 `InArrayFilter` 的 `filter` 方法为准。
...
params:
  demo:
    template: '{{in_array .role .role_list}}'
```

## 注意事项

### 定位
- 悬停命中文档路径：`tooltip-docs/python/key/template/value/in_array.md`
