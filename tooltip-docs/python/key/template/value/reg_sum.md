...
- 在 `params.*.template` 中生成 `reg_sum` 相关值。
...
### 参数
- 参数形式以实现类 `RegSumFilter` 的 `filter` 方法为准。
...
params:
  demo:
    template: '{{reg_sum .value}}'
```

## 注意事项

### 定位
- 悬停命中文档路径：`tooltip-docs/python/key/template/value/reg_sum.md`
