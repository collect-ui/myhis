...
- 在 `params.*.template` 中生成 `json_str` 相关值。
...
### 参数
- 参数形式以实现类 `JsonStr` 的 `filter` 方法为准。
...
params:
  demo:
    template: '{{json_str .data}}'
```

## 注意事项

### 定位
- 悬停命中文档路径：`tooltip-docs/python/key/template/value/json_str.md`
