# template function=current_date_format
...
- 在 `params.*.template` 中生成 `current_date_format` 相关值。
...
### 参数
- 调用签名：`func CurrentDateFormat(fmt string)`
...
params:
  demo:
    template: '{{current_date_format "2006-01-02"}}'
```

## 注意事项

### 定位
- 悬停命中文档路径：`tooltip-docs/go/key/template/value/current_date_format.md`
