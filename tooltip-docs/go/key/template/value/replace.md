...
- 在 `params.*.template` 中生成 `replace` 相关值。
...
### 参数
- 调用签名：`func Replace(source string, from string, to string)`
...
params:
  demo:
    template: '{{replace .name "-" "_"}}'
```

## 注意事项

### 定位
- 悬停命中文档路径：`tooltip-docs/go/key/template/value/replace.md`
