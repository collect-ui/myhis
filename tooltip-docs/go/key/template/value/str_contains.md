# template function=str_contains
...
- 在 `params.*.template` 中生成 `str_contains` 相关值。
...
### 参数
- 调用签名：`func StrContains(s, substr string)`
...
params:
  demo:
    template: '{{str_contains .content "ERROR"}}'
```

## 注意事项

### 定位
- 悬停命中文档路径：`tooltip-docs/go/key/template/value/str_contains.md`
