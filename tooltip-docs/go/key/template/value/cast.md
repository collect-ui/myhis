...
- 把值转换为目标类型（int/int64/float/float64）。
- 可用于 `params[*].template`、`check.template`、`enable`、`if_template` 等模板表达式位置。

## 常见用途

- 在 `params.*.template` 中生成 `cast` 相关值。
...
### 参数
- 调用签名：`func Cast(value any, dataType string)`
...
params:
  demo:
    template: '{{cast .page "int"}}'
```

## 注意事项

### 定位
- 悬停命中文档路径：`tooltip-docs/go/key/template/value/cast.md`
